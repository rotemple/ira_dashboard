...
dff = df.copy() 
f = px.scatter(dff.sort_values(by='created',ascending=True),x='id',y='impressions',hover_data=['description'], title="Impressions")
st.plotly_chart(f)

ff = px.scatter(dff.sort_values(by='created',ascending=True),x='id',y='clicks',hover_data=['description'], title="clicks")
st.plotly_chart(ff)
csv = convert_df(df)
